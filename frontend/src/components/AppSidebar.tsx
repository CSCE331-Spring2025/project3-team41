  ...
  function AppSidebarGroup({ group, buttons, collapsible }: Group) {
    return (
      <SidebarGroup key={group}>
        ...
        {buttons && <SidebarMenu>{buttons.map(AppSidebarButton)}</SidebarMenu>}
        {collapsible && (
          <SidebarMenu>{collapsible.map(AppSidebarCollapsible)}</SidebarMenu>
        )}
      </SidebarGroup>
    );
  }
    ...
    return (
      ...
      >
        <SidebarMenuItem>
          ...
          <CollapsibleContent>
            <SidebarMenuSub>{pages.map(AppSidebarPage)}</SidebarMenuSub>
          </CollapsibleContent>
        </SidebarMenuItem>
      </Collapsible>
    );
  ...
  function AppSidebarPage({ page, link }: Page) {
    return (
      <SidebarMenuSubItem key={page}>
        <SidebarMenuSubButton asChild isActive={currentPath == link.to}>
          <Link {...link}>{page}</Link>
        </SidebarMenuSubButton>
      </SidebarMenuSubItem>
    );
