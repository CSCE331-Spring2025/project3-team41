...
function RouteComponent() {
  ...
  function sendOrder() {
    ...
    const total = order.reduce(
      (acc, i) =>
        (acc +=
          i.quantity * calculateAdjustedPrice(i.item.price, weather?.temp)),
      0
    );
        ...
        >
          {fullMenu.map((item, index) => (
            <Button
              ...
              role="button"
              tabIndex={0}
              aria-label={`Menu item: ${item.item}. ${item.description}. Price ${item.price} dollars.`}
              onClick={() => {
                if (
                  !order.find(
                    (o_item) => o_item.item.item === item.item
            ...
            >
              <div className="flex flex-col gap-2">
                ...
                <div className="flex gap-2 justify-center">
                  {item.allergens.map((al: any, index: number) => (
          ...
          >
            ...
            {order
              .reduce(
                (acc, i) =>
                  (acc +=
                    i.quantity * calculateAdjustedPrice(
                      i.item.price,
                      weather?.temp
                    )),
                0
